  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h2 class="modal-title" id="exampleModalLabel">Tambah Data Cabang</h2>
      </div>
      <div class="modal-body">
        ...
        {{csrf_field()}}
            <div class="form-group{{$errors->has('nama_cabang') ? ' has-error' : ''}}">
                ...
                <input name="nama_cabang" type="text" class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp" placeholder="Nama Cabang"  value="{{old('nama_cabang')}}">
                @if($errors->has('nama_cabang'))
                    <span class="help-block">{{$errors->first('nama_cabang')}}</span>
                @endif
            </div>
  ...
  @section('script')
    ...
    $(document).ready(function() 
    {
        $('#datatable').DataTable({
            processing:true,
            serverside:true,
            ajax:"{{route('ajax.get.data.cabang')}}",
            columns:[
